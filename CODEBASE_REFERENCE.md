...
- `draw(screen, camera, dt)` — full experience: recolors on variable change, draws geometry + vectors + smoke particles, then the ImGui "Post-Processor" window and probe tooltip. Used only in the `VISUALIZER` state. `dt` (seconds, from `main.py`'s `clock.tick(60)`) drives smoke-particle advection only — everything else in `draw()` is per-frame-independent.
- `draw_geometry(camera)` — just the colored-mesh GL draw (position + color VBO), no ImGui, no smoke particles. Used by `main.py` during `SOLVING` so the live preview doesn't pop up its own window (or animate particles through a still-changing field) on top of the Solver Monitor. This is *why* smoke particles only ever animate post-solve, with no extra state-machine guard needed.
...
- **Rendering**: one dynamic `GL_POINTS` VBO (`GL_DYNAMIC_DRAW`, rebuilt every `step()`), same apply-transform/bind/draw/cleanup bracket as `draw_geometry()`/the vector glyphs.
