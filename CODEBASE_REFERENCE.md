...
| Mesh build frozen / stale stage label / Cancel not working | `mesher_panel.py` — thread loop, queues, `_drain()`; `mesher.py → mesh()`/`smooth_mesh()`'s `progress_queue`/`stop_event` checkpoints |
| Headless scripting / parametric studies | `api.py → run_solve()`, `geometry_helpers.py` |
...
```
EDITOR  →  PHYSICS  ⇄  MESHING  →  SOLVING  →  VISUALIZER
              ▲                                    │
              └────────────────────────────────────┘
```
...
**Since v1.10.1**, `PHYSICS ⇄ MESHING` mirrors `PHYSICS → SOLVING → VISUALIZER`'s threaded pattern: clicking Mesh/Remesh/Smooth Mesh no longer blocks the UI thread — it launches a `MesherPanel` (see that module's section below) and moves to `MESHING`, which always transitions straight back to `PHYSICS` once the background thread finishes, is cancelled, or errors (there's no `MESHING → SOLVING` path; you always land back in the panel to inspect/solve).
...
- **`AppContext`** — a plain dataclass holding what used to be a dozen bare local variables in `run_app()`'s closure: `camera`, `editor`, `gfx` (the shared `Renderer`, since v1.9.0), `state`, `vbos` (now `dict[str, VboHandle]`), `physicseditor`, `mesher`, `mesher_panel` (since v1.10.1), `solver` (kept alive across SOLVING and VISUALIZER, same as before), `solver_panel`, `visualizer`, `vis_mesher`, `live_field`. The old `screen`/`renderer` fields were removed in v1.9.0 — nothing outside `main.py` reads the pygame surface or ImGui backend anymore.
- **`update_editor(ctx)`** — EDITOR → PHYSICS on `ctx.editor.finished`; constructs `PhysicsEditor(...)`.
- **`update_physics(ctx)`** — dispatches on `ctx.physicseditor.pending_action` (a `PhysicsAction` enum from `physics_editor.py`, replacing the old four independent `mesh_requested`/`load_requested`/`load_visualization_requested`/`solve_requested` booleans — see that module's section below). Since v1.10.1, `MESH` and `SMOOTH_MESH` no longer run inline — they construct a `MesherPanel` (auto-starts its thread) and move to `MESHING`; `MESH` also clears `ctx.vbos` immediately first (`Mesher.upload_wireframe_bundles(ctx.vbos, {})`, reusing its delete-old-handles logic with an empty bundle dict) so a remesh builds live from an empty view instead of leaving the stale mesh on screen. `LOAD_MESH` stays in `PHYSICS`; `LOAD_VISUALIZATION` jumps straight to `VISUALIZER` (skipping `SOLVING` entirely); `SOLVE` builds `Solver`+`SolverPanel`+the live-preview `Visualizer` (`live_field`) and moves to `SOLVING`.
- **`update_meshing(ctx)`** (since v1.10.1) — MESHING → PHYSICS once `ctx.mesher_panel.state` leaves `'RUNNING'`. Structured identically to `update_solving()` below: while running, drains `panel.latest_bundles` (set by the *previous* frame's `render_meshing()` → `panel.draw()` → `_drain()`, same one-frame lag as the solver's `viz_snapshot`) and pushes it through `Mesher.upload_wireframe_bundles()` so the wireframe visibly builds in stages. On `'DONE'`, does the `has_mesh`/`physicseditor.mesher`/`ctx.mesher`/`loaded_mesh=None` bookkeeping that used to happen inline in the old synchronous `MESH` branch — but only for `mode='mesh'`; `mode='smooth'` leaves `has_mesh`/`mesher` alone since smoothing operates on an already-valid mesh in place. On `'CANCELLED'`/`'ERROR'`: a cancelled/errored **mesh** genuinely leaves incomplete geometry, so `has_mesh` is forced back to `False` and `physicseditor.mesher`/`ctx.mesher` to `None` (deliberate — the half-built wireframe stays visible as feedback, but the Solve/Smooth Mesh buttons correctly disappear); a cancelled/errored **smooth** leaves `has_mesh`/`mesher` untouched, because `smooth_mesh()`'s passes are atomic (build → filter → compare → commit-or-revert) so whatever committed before the stop is always a fully valid mesh.
...
- **`_apply_loaded_mesh_settings(physicseditor, loaded, vbos)`** (module-level) — rebuilds BC lines, meshing/solver scalar settings, `bc_spacing_map`/`refinement_zones`, and the wireframe preview VBOs from a loaded `.npz` dict. Shared by `LOAD_MESH` and `LOAD_VISUALIZATION` so returning to `PHYSICS` afterward always shows a consistent, populated state. Its VBO rebuild goes through `Mesher.upload_wireframe_bundles()` (see `mesher.py` below), which since v1.9.0 returns `VboHandle`s. **Bug fixed in v1.11.0:** this used to overwrite `physicseditor._bc_spacing` with `loaded['bc_spacing_map']` verbatim, trusting it to always carry all 4 known BC types — true for anything the GUI ever built, but a mesh built via the new `api.py → run_solve()` (or any hand-constructed `Mesher` that never passed `bc_spacing_map`) saves `{}`, which silently emptied `_bc_spacing` on load. With it empty, `physics_editor.py`'s "Linked spacing" checkbox's unlinked branch (`for bc_type in sorted(self._bc_spacing.keys())`) had nothing to iterate — no per-BC textboxes at all, and "Reset all to global" was a no-op for the same reason. Fixed by always backfilling every known BC type from `physicseditor.boundary_spacing` for any key missing from the loaded map, rather than trusting the loaded map's key set.
- **Render handlers** (`render_editor`/`render_physics`/`render_meshing`/`render_solving`/`render_visualizer`) are pure drawing since v1.9.0 — each passes `ctx.gfx` into the state object's `draw()`. The `viz_snapshot` drain that used to sit in `render_solving` (state mutation: consume the one-shot snapshot, re-upload the color VBO) moved into `update_solving`'s not-finished branch, so SOLVING is composed the same way as every other state: update mutates, render draws (`live_field.draw_geometry(gfx)` + walls `gfx.draw_vbo` + `solver_panel.draw(live_field=...)`). `render_meshing` (v1.10.1) follows the same split: three `gfx.draw_vbo` calls for the wireframe (`triangles`/`quads`/`walls` — the same three `PhysicsEditor.draw()` uses) plus `ctx.mesher_panel.draw()`, no state mutation.
...
### `mesher_panel.py` — Threaded Mesh Orchestration + Build Monitor UI (since v1.10.1)

Wraps a `Mesher` instance and runs `mesh()` or `smooth_mesh()` on a background thread, mirroring `solver_panel.py`'s pattern closely enough that most of that section's notes apply here too.

- Constructed as `MesherPanel(mesher, mode, smooth_kwargs=None)` where `mode` is `'mesh'` or `'smooth'` — one class, two thin call targets, rather than two near-duplicate panel classes.
- Two queues, matching the solver's split: `_progress_queue` (`maxsize=1`, drop-and-replace — carries `{'label': str, 'bundles': dict|None}`) and `_status_queue` (unbounded, but carries at most one terminal message per run: `'done'`/`'cancelled'`/`'error'`). Unlike the solver, `mesher.mesh()`/`smooth_mesh()` push directly onto `_progress_queue` themselves (see the `mesher.py` section below) — there's no per-iteration `step()` call for `_run()` to hook into from the outside, since meshing is one flat multi-stage method, not a loop with a clean per-iteration boundary. `_run()` (the thread body) owns only the terminal `_status_queue` messages, decided after `mesh()`/`smooth_mesh()` returns or raises — same separation of concerns as `SolverPanel` deciding `'converged'`/`'diverged'`/`'done'` in `_solve_loop`, not inside `Solver.step()`.
- `MeshingCancelled` (defined in `mesher.py`) is the only exception `_run()` treats as non-fatal — caught separately from a bare `except Exception`, which prints the full traceback (loud failure, never swallowed) and reports `'error'`.
- `_drain()` (called once per frame from `draw()`, main-thread only, same discipline as `SolverPanel._drain_queues()`) pulls the latest `{label, bundles}` into `self.stage_label`/`self.latest_bundles`, and the terminal message (if any) into `self.state`.
- `draw()` is much smaller than `SolverPanel.draw()` — no residual plots, no pause/step/run-to controls, just a status badge, the current stage label, an error message if `state == 'ERROR'`, and a Cancel button (`self.stop()`, sets `_stop_event`) while `state == 'RUNNING'`.
- **Why `mesh()` pushes its own progress instead of `MesherPanel` polling `Mesher` state from the outside:** `get_render_data()` (the wireframe snapshot) needs to run at specific points *inside* the 7-stage pipeline (e.g. right after `self.triangulation` is (re)built, before the next stage mutates related state) — there's no clean external vantage point to poll from, unlike the solver's `field_snapshot` property which the *outside* can read after any completed `step()`. This is a real structural difference from `SolverPanel`, not an inconsistency — `mesher.py`'s `progress_queue`/`stop_event` params are the pragmatic equivalent for a method shaped as one flat pipeline rather than a loop.
...
### `api.py` / `geometry_helpers.py` / `meshIO.py` — Scripting Entry Point (since v1.11.0)

The CFD core (`Mesher`, `Solver`) has always been fully headless — `test_holes.py`/`test_force_balance.py` proved that end to end with zero GL/pygame. What was missing was a documented public entry point; scripting meant hand-copying the test-file pattern. `cfdeditor/api.py` packages that pattern:

- `run_solve(lines, mesh_params, solver_params, smooth_params=None, verbose=True) -> SolveHandle` — builds a `Mesher(lines=lines, **mesh_params)`, calls `.mesh()` (and `.smooth_mesh(**smooth_params)` if given), runs `Solver(mesh_data, **solver_params).Solve()`, and returns a `SolveHandle(results, mesh_data)`. `mesh_params` is validated against an explicit allowlist; `solver_params` is validated against explicit required (`inlet_velocity`/`outlet_pressure`/`rho`/`viscosity`) and optional key sets — an unknown or missing key raises `TypeError` immediately (naming the offending keys) rather than surfacing as a bare `KeyError` from inside `Solver.__init__` or being silently ignored. `verbose=False` wraps the mesh/solve calls in `contextlib.redirect_stdout` to suppress `Mesher`'s/`Solver`'s unconditional per-stage `print()` output (useful for a parametric sweep of many cases) — implemented entirely inside `api.py`, no changes to `mesher.py`/`solver.py` themselves.
- Raises `SolveDiverged` if the solver never completed a single successful `step()` (i.e. diverged on iteration 0, before `finalize()` ever ran) — there is no valid `.results` in that case, matching the project's loud-failure convention rather than returning empty/garbage arrays.
- `SolveHandle` bundles `results` (a `SolverResults`) with `mesh_data` (the `solver_data_pipeline()` dict), plus convenience `.export_vtu(path)` / `.save_mesh(path)` / `.save_results(path)` methods — so a parametric sweep script can export each case without re-running the meshing pipeline just to get `mesh_data` back.
- `cfdeditor/geometry_helpers.py`: `rect_lines()` (promoted out of the two test files, which had duplicated copies with contradictory edge-order docstrings — the verified order is **0=bottom, 1=right, 2=top, 3=left**), plus new `circle_lines(cx, cy, radius, bc_type, n_segments)` and `polygon_lines(points, bc_map=None, default_bc="Wall")` for parametric-study geometry (e.g. a cylinder-in-channel sweep). Distinct from `constructor.py`'s "Geometry Helpers" (mesh-internal Bowyer-Watson/offset utilities) — this module is CAD-input construction for scripting, not mesh-internal math.
- `cfdeditor/meshIO.py` gained `save_results(results, filepath)` / `load_results(filepath)`, mirroring the existing `save_mesh_for_solver`/`load_mesh_for_solver` pair — a `SolverResults`-only `.npz` round-trip (`U`, `P`, `res_cont`, `res_mom`, `extra` via `allow_pickle`), for persisting per-case results separately from the mesh that produced them.
...
- UI: `PhysicsAction.SMOOTH_MESH` (`physics_editor.py`), button only shown when `self.has_mesh and self.mesher is not None` (a *loaded* `.npz` mesh sets `has_mesh` without ever constructing a `Mesher`, so the guard matters — this crashed once before the `mesher is not None` check was added). Passes/relaxation are in a "Smoothing settings" collapsing section next to Mesher settings/Refinement Zones, not inline on the button row. Since v1.10.1, `PhysicsAction.SMOOTH_MESH` launches a threaded `MesherPanel` (see that module's section above) rather than calling `smooth_mesh()` inline — `update_meshing()` (`app_state.py`) does the `rebuild_wireframe_vbos()`/`loaded_mesh=None` work once the panel reports done.

#### Phase 6 (opt-in, since v1.10.1): Threading hooks on `mesh()` and `smooth_mesh()`

Both methods gained optional `progress_queue`/`stop_event` parameters — default `None`, so every headless caller (`test_holes.py`, `test_force_balance.py`, any scripting use) is unaffected; only `MesherPanel` passes real ones.

- **`mesh()`**: pushes `{'label': str, 'bundles': dict|None}` onto `progress_queue` (a `queue.Queue(maxsize=1)`, drop-and-replace via the module-level `_push_latest()` helper) at each of its 7 internal stage transitions. **The label always describes the stage about to run, not the one that just finished** — this was a real bug in the first version: labels were pushed *after* each stage completed, so a slow stage (Steiner-point placement measured at ~39s on a fine mesh) sat there showing the *previous* stage's name for its entire duration. Fixed by moving each `_checkpoint()` call to the top of the stage it describes. A consequence: bundle-bearing and label-only pushes had to be **combined into one push per transition** rather than two back-to-back ones — on a `maxsize=1` queue, firing two pushes with no work in between lets the second silently discard the first before the main thread ever drains it, which would have dropped the boundary-layer wireframe snapshot entirely.
- **Cancellation granularity:** `stop_event.is_set()` is checked at all 7 stage boundaries, **plus inside `create_steiner_points()`'s own Poisson-disk `while active:` loop** (passed through as a third param on that method). This inner check is not optional/cosmetic — Steiner-point placement is by far the most likely single bottleneck on a fine mesh (the ~39s case above), and without a check *inside* that stage, Cancel would have to wait out the entire stage before taking effect, which defeats the point of having Cancel at all. Verified empirically: without the inner check, `stop()` took ~39s to actually halt the thread; with it, ~0.3s.
- **`smooth_mesh()`**: `stop_event` is checked **between** passes (top of the `for` loop), not mid-pass — each pass is already atomic (build candidate → filter → compare → commit-or-revert), so a between-pass stop always leaves a fully valid triangulation. No `MeshingCancelled` is raised here (unlike `mesh()`); it's just an early `break`, identical in shape to the existing "pass regressed past tolerance" early-exit.
- `MeshingCancelled` (module-level exception in `mesher.py`) is what `mesh()`/`create_steiner_points()` raise on a stop; `MesherPanel._run()` catches it separately from a generic `Exception` (see that module's section above).
