...
- **Smooth Mesh (opt-in Laplacian smoothing)**: A "Smooth Mesh" button relaxes interior Steiner points toward their Delaunay-neighbour centroid and re-triangulates, evening out the size/shape discontinuity at the ring/interior seam. Boundary-layer and ring points are always frozen — only Steiner points move — so it's additive and reversible (re-mesh to undo). A pass is accepted unless it drops the worst seam triangle's minimum angle by more than a small tolerance below the best value seen so far, since some triangle elsewhere drifting slightly is normal Delaunay-retriangulation noise, not a real regression. Passes/relaxation factor are configurable under "Smoothing settings".
- **Threaded mesh generation with a live build preview**: Mesh/Remesh and Smooth Mesh both run on a background thread (mirroring the solver's own threaded live monitor) instead of blocking the UI. A "Mesher" panel shows the current build stage and a Cancel button; the wireframe visibly builds up in stages (boundary layers appear, then the interior triangulation) rather than popping in all at once. Cancelling a fresh Mesh leaves `has_mesh` correctly `False` (the geometry is genuinely incomplete); cancelling Smooth Mesh keeps whatever passes already committed, since each smoothing pass is atomic.
